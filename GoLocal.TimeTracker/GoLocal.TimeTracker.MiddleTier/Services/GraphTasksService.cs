...
{
    public class GraphTasksService : GraphServiceBase
    {
        ...
        public async Task<IList<JToken>> GetUserTasksAsync(DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                var requestUrl = _timeTrackerOptions.GraphBetaRequestUrl + "/me/outlook/tasks?filter=startDateTime/dateTime ge '" + startDate.ToString("s") + "' and startDateTime/dateTime le '" + endDate.ToString("s") + "'";
