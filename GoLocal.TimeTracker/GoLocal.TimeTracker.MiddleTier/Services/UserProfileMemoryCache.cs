...
{
    public class UserProfileMemoryCache : ICacheService<UserProfile>
    {
        private readonly ILogger _logger;
        private readonly TimeTrackerOptions _timeTrackerOptions;
        private readonly IMemoryCache _memoryCache;
        private readonly int _cacheTime;
        private const string _cacheKey = "_up";

        public UserProfileMemoryCache(
            ILogger<UserProfileMemoryCache> logger,
            ...
            IMemoryCache memoryCache)
        {
            ...
            _cacheTime = _timeTrackerOptions.UserProfileCacheMinutes;
        }

        public async Task<UserProfile> TryRetrieveFromCacheAsync(UserProfile items, string key)
        {
            key = key + _cacheKey;
            ...
            items = JsonConvert.DeserializeObject<UserProfile>(entryFromCache);

            return items;
        }

        public async Task SaveToCacheAsync(UserProfile items, string key)
        {
            key = key + _cacheKey;
