...
{
    public class DirectReportsMemoryCache : ICacheService<ListCollectionPage<GraphResultItem>>
    {
        private readonly ILogger _logger;
        private readonly TimeTrackerOptions _timeTrackerOptions;
        private readonly IMemoryCache _memoryCache;
        private readonly int _cacheTime;
        private const string _cacheKey = "_dr";

        public DirectReportsMemoryCache(
            ILogger<DirectReportsMemoryCache> logger,
        ...
        public async Task<ListCollectionPage<GraphResultItem>> TryRetrieveFromCacheAsync(ListCollectionPage<GraphResultItem> items, string key)
        {
            key = key + _cacheKey;
            ...
            items = JsonConvert.DeserializeObject<ListCollectionPage<GraphResultItem>>(entryFromCache);

            return items;
        }

        public async Task SaveToCacheAsync(ListCollectionPage<GraphResultItem> items, string key)
        {
            key = key + _cacheKey;
