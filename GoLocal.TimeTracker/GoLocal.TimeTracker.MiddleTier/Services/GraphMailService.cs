...
{
    public class GraphMailService : GraphServiceBase
    {
        ...
        public async Task<List<GraphResultItem>> GetMailItemsAsync(DateTime startDate, DateTime endDate)
        {
            try
            {
				...
				options.Add(new QueryOption("$filter", @"ReceivedDateTime ge " + startDate.ToString("yyyy-MM-dd'T'HH':'mm':'ss'Z'") + " and " +
				"receivedDateTime le " + endDate.ToString("yyyy-MM-dd'T'HH':'mm':'ss'Z'")));


				var graphResponse = await GraphClient.Me.MailFolders.Inbox.Messages.Request(options).GetAsync();
				...
				sentOptions.Add(new QueryOption("$filter", @"LastModifiedDateTime ge " + startDate.ToString("yyyy-MM-dd'T'HH':'mm':'ss'Z'") + " and " +
                ...
                // createdDateTime gt
                var sentGraphResponse = await GraphClient.Me.MailFolders.SentItems.Messages.Request(sentOptions).GetAsync();
