...
namespace GoLocal.TimeWise.AzureFunctions.Services
{
    public class GraphAppUserService : GraphAppServiceBase
    {
        ...
        public async Task<List<GraphResultItem>> GetMemberOfAsync(string objectIdentifier)
        {
            try
            {
                ...
                {
                    graphResultsItems = await GetMemberOfAsync(objectIdentifier);
                }
                else // Uses SharePoint group to get group membership
                {
                    graphResultsItems = await GetSiteListMemberOfAsync(objectIdentifier);
                }
        ...
        public async Task SendEmailAsync(string userObjectIdentifier, string sendTo, string subject, string emailTemplate)
        {
            ...
            var email = new Message
            {
                Body = new ItemBody
                {
                    Content = System.IO.File.ReadAllText("C:\\Users\\Fady\\source\\repos\\Time_Tracker\\GoLocal.TimeTracker\\GoLocal.TimeW#ise.AzureFunctions\\wwwroot" + emailTemplate),
        ...
        private async Task<List<GraphResultItem>> GetSiteListMemberOfAsync(string objectIdentifier)
        {
            try
            {
                // Get users's email.
                //var email = User.Identity.Name ?? User.FindFirst("preferred_username").Value;
                var userInfoResults = await GetUserBasicAsync(objectIdentifier);
                var email = userInfoResults.Properties["Mail"].ToString() ?? userInfoResults.Properties["Upn"].ToString();
