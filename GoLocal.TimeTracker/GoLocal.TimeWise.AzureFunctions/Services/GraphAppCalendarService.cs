...
{
    public class GraphAppCalendarService : GraphAppServiceBase
    {
        ...
        public async Task<List<GraphResultItem>> GetCalendarEventsAsync(DateTime startDate, DateTime endDate)
        {
            try
            {
                ...
                options.Add(new QueryOption("startDateTime", TimeZoneInfo.ConvertTimeToUtc(startDate, localTimeZone).ToString("yyyy-MM-dd'T'HH':'mm':'ss")));
                options.Add(new QueryOption("endDateTime", TimeZoneInfo.ConvertTimeToUtc(endDate, localTimeZone).ToString("yyyy-MM-dd'T'HH':'mm':'ss")));

                // Get the user object identifier

                var graphResponse = await GraphAppClient.Users[_userObjectIdentifier].CalendarView.Request(options).GetAsync();
                ...
                foreach (var item in eventsResults)
                {
                    ...
                    {
                        String localStartStr = TimeZoneInfo.ConvertTimeFromUtc(startTime, definedZone).ToString();
                        resultsItemProperties.Add("Start", localStartStr);
                        String localEndStr = TimeZoneInfo.ConvertTimeFromUtc(endTime, definedZone).ToString();
